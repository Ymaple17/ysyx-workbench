...
class NPC_IO(conf: CoreConfig) extends Bundle {
  private val exposeSimState = conf.npc || conf.ysyxsoc
  ...
  val commit_valid    = if (exposeSimState) Some(Output(Bool())) else None
  val commit_pc       = if (exposeSimState) Some(Output(UInt(32.W))) else None
  val commit_mem_addr = if (exposeSimState) Some(Output(UInt(32.W))) else None
  val commit_is_load  = if (exposeSimState) Some(Output(Bool())) else None
  val commit_valid1    = if (exposeSimState) Some(Output(Bool())) else None
  val commit_pc1       = if (exposeSimState) Some(Output(UInt(32.W))) else None
  val commit_mem_addr1 = if (exposeSimState) Some(Output(UInt(32.W))) else None
  val commit_is_load1  = if (exposeSimState) Some(Output(Bool())) else None
  val commit_valid2    = if (exposeSimState) Some(Output(Bool())) else None
  val commit_pc2       = if (exposeSimState) Some(Output(UInt(32.W))) else None
  val commit_mem_addr2 = if (exposeSimState) Some(Output(UInt(32.W))) else None
  val commit_is_load2  = if (exposeSimState) Some(Output(Bool())) else None
  val commit_valid3    = if (exposeSimState) Some(Output(Bool())) else None
  val commit_pc3       = if (exposeSimState) Some(Output(UInt(32.W))) else None
  val commit_mem_addr3 = if (exposeSimState) Some(Output(UInt(32.W))) else None
  val commit_is_load3  = if (exposeSimState) Some(Output(Bool())) else None
  val arch_rdata      = if (exposeSimState) Some(Output(Vec(32, UInt(32.W)))) else None
...
class ysyx_25020039(val coreConfig: CoreConfig) extends Module {
  ...
  if (coreConfig.npc || coreConfig.ysyxsoc) {
