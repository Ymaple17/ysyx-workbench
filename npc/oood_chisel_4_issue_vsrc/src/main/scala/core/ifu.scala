...
class IFU(val conf: CoreConfig) extends Module{
  ...
  when(fetchPc >= "ha00055c0".U && fetchPc <= "ha00055dc".U) {
    printf(p"[OOOD-RTL-IFU] pc=0x${Hexadecimal(fetchPc)} inst0=0x${Hexadecimal(sourceInst(0))} " +
      p"bp_valid=${bpu.io.bp_valid} bp_taken=${bpu.io.bp_taken} " +
      p"bp_target=0x${Hexadecimal(bpu.io.bp_target)} seq=0x${Hexadecimal(seqNext)} " +
      p"next=0x${Hexadecimal(predictedNext)} state=${state} in_valid=${io.in.valid} " +
      p"flush=${io.is_flush}\n")
  }
