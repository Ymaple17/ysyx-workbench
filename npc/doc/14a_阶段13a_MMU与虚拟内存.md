...
- **当前参考核**：**未做虚拟内存**（现状：阶段 12 单核性能主线仍使用直接物理地址，无 satp/PTW/TLB，无 U/S 模式）。本章系统能力继续后置。
...
---
**前置**：阶段 12 的单核 IPC 1.3 主线完成验收后；当前仍是直接物理地址裸机核。确实有跑 OS/用户程序需求时再动。  
...
→ [14b_阶段13b_多核与一致性.md](14b_阶段13b_多核与一致性.md)：继续系统扩展科普；或回到 [README](README.md)。
