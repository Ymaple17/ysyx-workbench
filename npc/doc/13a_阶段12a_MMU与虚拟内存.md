...
- **当前参考核**：**未做**（现状：直接物理地址访存，无 satp/PTW/TLB，无 U/S 模式）。当前参考核仍停在阶段 10m，阶段 11 是 IPC 1.0 讲义目标；本章系统能力继续后置。
- **后续扩展**：阶段 11 单核 IPC 主线收口后，本章再逐步加入 iTLB/dTLB、PTW、权限检查与异常提交。
- **验收方式**：定向覆盖页表两级遍历、TLB hit/miss、跨页取指/访存、权限错误、sfence.vma 和 flush；再跑能使用虚拟地址的 OS/用户态 workload 与参考模型。

---
**前置**：讲义路线先完成阶段 11 的单核 IPC 1.0 主线；当前参考核源码仍到 10m，且是直接物理地址裸机核。确实有跑 OS/用户程序需求时再动。  
...
→ [13b_阶段12b_多核与一致性.md](13b_阶段12b_多核与一致性.md)：继续系统扩展科普；或回到 [README](README.md)。
