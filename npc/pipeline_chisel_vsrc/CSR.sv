// Generated by CIRCT firtool-1.114.1
module CSR(
  input         clock,
                io_irq,
  input  [7:0]  io_irq_no,
  input  [31:0] io_irq_pc,
  input  [11:0] io_read_raddr,
  output [31:0] io_read_rdata,
                io_read_mtvec,
                io_read_mepc,
  input  [31:0] io_write_wdata,
  input  [11:0] io_write_waddr,
  input         io_write_wen
);

  reg  [31:0]      rf_0;
  reg  [31:0]      rf_1;
  reg  [31:0]      rf_2;
  reg  [31:0]      rf_3;
  reg  [31:0]      rf_4;
  reg  [31:0]      rf_5;
  wire [7:0][31:0] _GEN =
    {{rf_0}, {rf_0}, {rf_5}, {rf_4}, {rf_3}, {rf_2}, {rf_1}, {rf_0}};
  always @(posedge clock) begin
    automatic logic [2:0] in_waddr;
    automatic logic       _GEN_0;
    in_waddr =
      io_write_waddr == 12'hF12
        ? 3'h5
        : io_write_waddr == 12'hF11
            ? 3'h4
            : io_write_waddr == 12'h342
                ? 3'h3
                : io_write_waddr == 12'h341 ? 3'h2 : {2'h0, io_write_waddr == 12'h305};
    _GEN_0 = io_write_wen & ~io_irq;
    rf_0 <= _GEN_0 & in_waddr == 3'h0 ? io_write_wdata : 32'h1800;
    if (_GEN_0 & in_waddr == 3'h1)
      rf_1 <= io_write_wdata;
    if (io_irq) begin
      rf_2 <= io_irq_pc;
      rf_3 <= {24'h0, io_irq_no};
    end
    else begin
      if (_GEN_0 & in_waddr == 3'h2)
        rf_2 <= io_write_wdata;
      if (_GEN_0 & in_waddr == 3'h3)
        rf_3 <= io_write_wdata;
    end
    rf_4 <= _GEN_0 & in_waddr == 3'h4 ? io_write_wdata : 32'h79737978;
    rf_5 <= _GEN_0 & in_waddr == 3'h5 ? io_write_wdata : 32'h25020039;
  end // always @(posedge)
  assign io_read_rdata =
    _GEN[io_read_raddr == 12'hF12
           ? 3'h5
           : io_read_raddr == 12'hF11
               ? 3'h4
               : io_read_raddr == 12'h342
                   ? 3'h3
                   : io_read_raddr == 12'h341 ? 3'h2 : {2'h0, io_read_raddr == 12'h305}];
  assign io_read_mtvec = rf_1;
  assign io_read_mepc = rf_2;
endmodule

