...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // ★ div/rem 必须显式处理除零与溢出(否则 C 里是 UB：x86 会 SIGFPE 崩溃)
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = ((sword_t)src2 == 0) ? (word_t)-1 : (((sword_t)src1 == INT32_MIN && (sword_t)src2 == -1) ? src1 : (word_t)((sword_t)src1 / (sword_t)src2)));
	INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = (src2 == 0) ? (word_t)-1 : (src1 / src2));
	INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(rd) = src1 & src2);
	INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(rd) = src1 | src2);
	INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor    , R, R(rd) = src1 ^ src2);
	INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = ((sword_t)src2 == 0) ? src1 : (((sword_t)src1 == INT32_MIN && (sword_t)src2 == -1) ? 0 : (word_t)((sword_t)src1 % (sword_t)src2)));
...
int isa_exec_once(Decode *s) {
  s->isa.inst = inst_fetch(&s->snpc, 4);
	IFDEF(CONFIG_ITRACE, itrace_inst(s->pc, s->isa.inst));
