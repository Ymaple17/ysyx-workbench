...
class DIV extends Module{
    ...
    if (OoOParams.FAST_DIV_ENABLE) {
        val isSigned = io.op === ALU_DIV || io.op === ALU_REM
        val isRemainder = io.op === ALU_REM || io.op === ALU_REMU
        val divideByZero = io.b === 0.U
        val safeUnsignedDivisor = Mux(divideByZero, 1.U(32.W), io.b)
        val safeSignedDivisor = Mux(divideByZero, 1.S(32.W), io.b.asSInt)
        val unsignedQuotient = io.a / safeUnsignedDivisor
        val unsignedRemainder = io.a % safeUnsignedDivisor
        val signedQuotient = (io.a.asSInt / safeSignedDivisor).asUInt
        val signedRemainder = (io.a.asSInt % safeSignedDivisor).asUInt
        val normalResult = Mux(isSigned,
          Mux(isRemainder, signedRemainder, signedQuotient),
          Mux(isRemainder, unsignedRemainder, unsignedQuotient))

        io.req_ready := true.B
        io.busy := false.B
        io.result_valid := io.req_valid && !io.kill
        io.result := Mux(divideByZero,
          Mux(isRemainder, io.a, "hffff_ffff".U), normalResult)
    } else {
        ...
        io.result := Mux(div_by_zero,
          Mux(is_rem, a_latched, "hffff_ffff".U),
          Mux(is_rem, Mux(a_neg, (~rem) + 1.U, rem),
        ...
        when(io.kill) {
            ...
            when(io.req_valid) {
                ...
                b_abs := Mux(io.op === ALU_DIVU || io.op === ALU_REMU, io.b,
                  Mux(io.b(31), (~io.b) + 1.U, io.b))
