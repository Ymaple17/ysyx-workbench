...
class BPU_IO extends Bundle {
    ...
    val update_is_call = Input(Bool()) //被解析的指令是否为函数调用（JALR x1, x1, imm）
    val update_is_ret = Input(Bool()) //被解析的指令是否为函数
}
...
class BPU(val conf: CoreConfig) extends Module {
    ...
    val ghr = RegInit(0.U(log2Ceil(BHT_SIZE).W)) //全局历史寄存器（7位，与索引同宽）
    ...
    //RAS
    // call：rd==x1 且 rs1 != x1（通过指针/寄存器跳转的函数调用）
    // ret：rs1 == x1；注意 jalr x1, 0(x1) 同时满足两者，按 ret 处理（ret 优先）
    val is_call = is_jalr && inst(11,7) === 1.U && inst(19,15) =/= 1.U
    val is_ret = is_jalr && inst(19,15) === 1.U
