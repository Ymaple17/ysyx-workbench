...
extract_microbench_csv() {
  ...
  local tage_use_alt bimodal_selected control_direct_complete store_direct_complete tage_alloc itage_hit itage_alloc loop_predict_hit
  local alu1_issues dual_alu_issues fu_refill rs_cdb_wake_issue rs_fresh_issue lsu_addr_refill wb_age_reorders
  ...
  store_direct_complete="$(extract_metric "${log}" "Store Direct Complete")"
  ...
  loop_predict_hit="$(extract_metric "${log}" "Loop Predictor Hits")"
  ...
  rs_fresh_issue="$(extract_metric "${log}" "RS Fresh Issue")"
