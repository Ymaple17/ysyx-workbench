...
class Core(val conf: CoreConfig) extends Module {
  ...
  val rob    = Module(new WideROBCompat(conf))
  ...
  when(d_bru_valid && d_bru_bits.pc >= "ha00055c0".U &&
      d_bru_bits.pc <= "ha00055dc".U) {
    printf(p"[OOOD-RTL-BRU] pc=0x${Hexadecimal(d_bru_bits.pc)} jump=${d_bru_bits.signals.exu.jump} " +
      p"pc_src=${Hexadecimal(pc_src)} is_ch=${is_ch} pred=${predict_taken} " +
      p"bp_valid=${bru_resolve_bits.bp_valid} bp_target=0x${Hexadecimal(bru_resolve_bits.bp_target)} " +
      p"correct=0x${Hexadecimal(correct_pc)} out_valid=${bru_resolve_out_valid} " +
      p"out_ready=${exu_bru.io.out.ready} mis=${mis_predict}\n")
  }
  ...
  for (i <- 0 until 32) {
    // The probe is sampled after the retire edge. Use the commit-state mirror
    // here so a same-cycle writeback is visible at the architectural boundary.
    io.arch_rdata(i) := Mux(i.U === 0.U, 0.U, arch_rf(i))
  }
