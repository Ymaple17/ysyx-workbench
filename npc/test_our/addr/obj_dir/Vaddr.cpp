...
std::unique_ptr<VerilatedTraceConfig> Vaddr::traceConfig() const {
...
void Vaddr___024root__trace_init_top(Vaddr___024root* vlSelf, VerilatedVcd* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    Vaddr___024root__trace_init_top(vlSelf, tracep);
...
VL_ATTR_COLD void Vaddr::trace(VerilatedVcdC* tfp, int levels, int options) {
    if (tfp->isOpen()) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vaddr::trace()' shall not be called after 'VerilatedVcdC::open()'.");
    }
    if (false && levels && options) {}  // Prevent unused
    tfp->spTrace()->addModel(this);
    tfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vaddr___024root__trace_register(&(vlSymsp->TOP), tfp->spTrace());
}
