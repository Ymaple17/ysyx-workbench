...
---
**前置**：阶段 9 已完成可维护性整理；有跑 OS/用户程序需求时再动。  
**目标**：给核加 **RISC-V SV32 分页**（页表遍历 + TLB + 权限检查），支持用户态/内核态切换。  
**仓库状态**：**未做**（现状：直接物理地址访存，无 satp/PTW/TLB，无 U/S 模式）。

> 这是「入门级超标量」之外最大的系统能力。没有它，核只能跑裸机（AM/cpu-tests）；有了它才能跑 Linux 类系统。教学核可选，但值得知道结构。
