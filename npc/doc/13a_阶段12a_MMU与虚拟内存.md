...
- **当前参考核**：**未做虚拟内存**（现状：阶段 11d 单核仍使用直接物理地址，无 satp/PTW/TLB，无 U/S 模式）。Stage11 的 IPC 主线已收口，本章系统能力继续后置。
...
---
**前置**：阶段 11 的单核 IPC 1.0 主线已经在参考核收口；当前仍是直接物理地址裸机核。确实有跑 OS/用户程序需求时再动。  
