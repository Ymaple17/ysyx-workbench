...
class DIV extends Module{
    ...
    val s_IDLE :: s_PREP :: s_BUSY :: Nil = Enum(3)
    ...
    val requestA = Reg(UInt(32.W))
    val requestB = Reg(UInt(32.W))
    val requestOp = Reg(UInt(5.W))
    ...
    when(io.kill) {
    ...
    }.elsewhen(state === s_PREP) {
            val is_signed = requestOp === ALU_DIV || requestOp === ALU_REM
            a_abs := Mux(is_signed && requestA(31), (~requestA) + 1.U, requestA)
            b_abs := Mux(is_signed && requestB(31), (~requestB) + 1.U, requestB)
            a_neg := is_signed && requestA(31)
            b_neg := is_signed && requestB(31)
            is_rem := requestOp === ALU_REM || requestOp === ALU_REMU
            div_by_zero := requestB === 0.U
            rem := 0.U
            quo := 0.U
            cnt := 32.U
            state := s_BUSY
        ...
        when(cnt =/= 0.U) {
            ...
            val bitIndex = (cnt - 1.U)(4, 0)
            val shifted = Cat(rem(30, 0), a_abs(bitIndex))
            rem := shifted
            when(shifted >= b_abs) {
                rem := shifted - b_abs
                quo := quo | (1.U(32.W) << bitIndex)
            }
            cnt := cnt - 1.U
        }
        when(cnt === 0.U) {
