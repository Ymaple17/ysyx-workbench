...
import unit.{BPU, FixedPeriodPredictor}
...
class IFU_IDU_IO extends Bundle{
  ...
  val bp_meta = Output(UInt(FixedPeriodPredictor.MetaWidth.W))
}
...
class IFU_IO(xlen: Int) extends Bundle{
  ...
  val bpu_update_meta = Input(UInt(FixedPeriodPredictor.MetaWidth.W))
...
class IFU(val conf: CoreConfig) extends Module{
    ...
    val pc_init = conf.resetVector.U(32.W)
    ...
    io.imem.rready := work || io.is_flush || (state === s_FLUSH)
    ...
    bpu.io.update_meta := io.bpu_update_meta
    ...
    io.out.bits.bp_meta := bpu.io.bp_meta
    ...
    val responseValid = io.imem.rvalid
    ...
    ready := responseValid && (state =/= s_FLUSH)
    idle := io.in.valid && io.imem.arready && !io.is_flush && bpu.io.init_done
    ...
    if(conf.statistics){
      ...
      PM(conf, clock, EVENT_IFU_NO_PC, 1.U, !reset.asBool && !io.in.valid)
      PM(conf, clock, EVENT_IFU_AR_BLOCK, 1.U,
        state === s_IDLE && io.in.valid && !io.is_flush && bpu.io.init_done && !io.imem.arready)
      PM(conf, clock, EVENT_IFU_FAST_BACKPRESSURE, 1.U,
        state =/= s_FLUSH && io.in.valid && io.imem.rvalid && !io.out.ready)
      PM(conf, clock, EVENT_IFU_FLUSH_WAIT, 1.U, state === s_FLUSH)
    }
}
