...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11111 11", jal    , J, s->dnpc = s->pc; s->dnpc += imm; R(rd) = s->pc+4);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    ,    B, const bool taken = src1 == src2; if (taken) { s->dnpc = s->pc + imm; });
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    ,    B, const bool taken = src1 != src2; if (taken) { s->dnpc = s->pc + imm; } );
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    ,    B, const bool taken = (sword_t)src1 < (sword_t)src2; if (taken) { s->dnpc = s->pc + imm; });
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    ,    B, const bool taken = (sword_t)src1 >= (sword_t)src2; if (taken) { s->dnpc = s->pc + imm; });
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   ,    B, const bool taken = src1 < src2; if (taken) { s->dnpc = s->pc + imm; });
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   ,    B, const bool taken = src1 >= src2; if (taken) { s->dnpc = s->pc + imm; });
  INSTPAT_END();
