...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //J
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s -> dnpc = s -> pc + imm;IFDEF(CONFIG_ITRACE,{if(rd==1){trace_func_call(s->pc,s->dnpc,false);}});R(rd) = s->pc+4);
  //B
  ...
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu, R, R(rd) = ((sword_t)src1 * src2) >> (sizeof(sword_t) * 4));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu , R, R(rd) = (src1 * src2) >> (sizeof(word_t) * 4));
