...
| 11 | [11a](11a_阶段10a_独立SQ与StoreBuffer.md) → [11b](11b_阶段10b_访存层次与非阻塞缓存.md) → [11c](11c_阶段10c_宽提交与多CDB.md) → [11d](11d_阶段10d_MMU与虚拟内存.md) → [11e](11e_阶段10e_高级分支预测.md) → [11f](11f_阶段10f_多核与一致性.md) | **阶段 10 重型结构扩展**（确认瓶颈后再做） |

**参考实现水位（≠ 你的学习勾选）**：已到 **阶段 8d 教学参考核**（多 FU、2 宽 rename/dispatch、checkpoint 恢复、ROB-backed StoreQueue）；阶段 9 是待做的性能画像与调优。

## 原则

1. 本阶段 **cpu-tests + difftest 自测绿** 再往下。  
2. **2d 不过 → 不做 3b**；**3a 不过 → 不做 3c**；**3c 绿 → 3d**；**3d 绿 → 4a/4b**；**4b 绿 → 4c**；**4c 绿 → 5a/5b**；**6 绿 → 7（先扫基线）→ 8a–8d（超标量）→ 9（性能画像）→ 10（重型扩展）**。  
