...
static FILE *commit_trace = nullptr;
static bool commit_trace_initialized = false;


extern "C" void sim_exit(){
  if (commit_trace != nullptr) {
    fclose(commit_trace);
    commit_trace = nullptr;
  }
...
static void execute(uint64_t n) {
  if (!commit_trace_initialized) {
    ...
    const char *trace_path = getenv("NPC_COMMIT_TRACE");
    if (trace_path != nullptr && trace_path[0] != '\0') {
      commit_trace = fopen(trace_path, "w");
      if (commit_trace == nullptr) {
        perror("NPC_COMMIT_TRACE");
      }
    }
  }
  ...
  for (; n > 0; n--) {
    if (!Verilated::gotFinish()) {
      ...
      bool wbu_valid = top->io_simCommit_valid;
      bool reset = top->reset;

      if (wbu_valid && !reset && rst_done) {
        hang_counter = 0;
        word_t commit_pc = top->io_simCommit_pc;
        if (commit_trace != nullptr) {
          fprintf(commit_trace, "%08x\n", commit_pc);
        }
        ...
        word_t wbu_alu = top->io_simCommit_aluResult;
        uint32_t inst = pmem_read(commit_pc, 4);
        bool is_store = ((inst & 0x7f) == 0x23);
        uint8_t wbu_reg_write_sel = top->io_simCommit_regWriteSel;
