...
static uint32_t keymap[256] = {};//存储sdl扫描码
...
static void key_enqueue(uint32_t am_scancode) {//入列
  key_queue[key_r] = am_scancode;
  key_r = (key_r + 1) % KEY_QUEUE_LEN;
  Assert(key_r != key_f, "key queue overflow!");
}

static uint32_t key_dequeue() {//出列
...
void init_i8042() {
  ...
  i8042_data_port_base[0] = _KEY_NONE;//init 无按键
...
#endif
  IFNDEF(CONFIG_TARGET_AM, init_keymap());//按键映射表
}
